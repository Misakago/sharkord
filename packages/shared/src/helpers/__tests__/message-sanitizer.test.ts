...
describe('isEmptyMessage', () => {
  ...
  test('should return true for unsupported video tags', () => {
    expect(isEmptyMessage('<video src="video.mp4"></video>')).toBe(true);
    expect(isEmptyMessage("<video><source src='v.mp4'></video>")).toBe(true);
  });

  test('should return true for unsupported audio tags', () => {
    expect(isEmptyMessage('<audio src="audio.mp3"></audio>')).toBe(true);
    expect(isEmptyMessage("<audio><source src='a.mp3'></audio>")).toBe(true);
  });

  test('should return true for iframe tags', () => {
    expect(isEmptyMessage('<iframe src="page.html"></iframe>')).toBe(true);
...
describe('removeEmojiElements', () => {
  ...
  test('should remove custom emoji img with class on parent span', () => {
    const html =
      '<p><span data-name="cool_doge" class="emoji-image" data-type="emoji"><img src="https://cdn.example.com/public/Cool%20Doge.gif" draggable="false" loading="lazy" align="absmiddle" alt="cool_doge emoji" /></span></p>';
    expect(removeEmojiElements(html)).toBe('<p></p>');
  });
...
describe('removeCommandElements', () => {
  test('should remove a command element', () => {
    const html = `<command data-plugin-id="mikotord-music-bot" data-plugin-logo="https://i.imgur.com/uVBNUK9.png" data-command="stop" data-args='[]' data-status='completed' data-response=''></command>`;
  ...
  test('should remove command element and preserve surrounding content', () => {
    const html = `<p>Hello</p><command data-plugin-id="mikotord-music-bot" data-command="stop" data-args='[]' data-status='completed' data-response=''></command><p>World</p>`;
